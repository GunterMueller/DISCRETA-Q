...
{
  printf("\nSyntax:\n\n");
  printf("discretaqsolve -c0factor [int] -bkz [int] [int] -file [filename] [-silent]\n\n");
  printf("discretaqsolve -c0factor [int] -iterate [int] -file [filename] [-silent]\n\n");
  printf("Examples:\n\n");
  printf("discretaqsolve -c0factor 1000 -bkz 80 18 -file equations.txt\n\n");
  printf("discretaqsolve -c0factor 1000 -iterate 10 -file equations.txt\n\n");
  fflush(stdout);
  return 1;
}
...
for(i = 0; i < argc; i++)
{
  if(strcmp(argv[i],"-c0factor")==0)
  {
    if (i+1 == argc)
    {
      printf("ERROR: what is c0factor?\n");
      return 1;
    }
    factor_input = atol(argv[i+1]);
  }
  ...
  if(strcmp(argv[i],"-iterate")==0)
  {
    if (i+1 == argc)
    {
      printf("ERROR: what is the iterante number?\n");
      return 1;
    }
    iterate = 1;
  ...
  if(strcmp(argv[i],"-bkz")==0)
  {
    if (i+1 == argc)
    {
      printf("ERROR: what is beta?\n");
      return 1;
    }
    if (i+2 == argc)
    {
      printf("ERROR: what is p?\n");
      return 1;
    }
    iterate = 0;
